...
func ПроверитьДанныеАаутентификации(req *http.Request) (string, uuid.UUID) {
	// var ЗаголвокJWT string
	// ЗаголвокJWT := req.Header.Get("X-UAT")

	// UID := req.Header.Get("X-UID")
	// ЗаголвокИдКлиента, err := uuid.Parse(UID)
	// if err != nil {
	// 	Ошибка("  %+v \n", err)
	// }

	var КукиИдКлиента uuid.UUID
	var КукиJWT string

	cookieUuid, err := req.Cookie("uuid")
	if err != nil {
		Ошибка("в куках нет uuid  %+v \n", err)
		// ИД = uuid.New()
	} else {
		Инфо("Читаем куки uuid %+v \n", cookieUuid)
		КукиИдКлиента, err = uuid.Parse(cookieUuid.Value)
		Инфо("КукиИдКлиента %+v \n", КукиИдКлиента)
	...
	if cookieJWT, err := req.Cookie("jwt"); err != nil {
		Ошибка("в куках нет jwt %+v мнее кажеться тут это лишнее \n", err)

	} else {
		КукиJWT = cookieJWT.Value
		Инфо(" Сохраняем данные JWT для идентификации %+v  \n", КукиJWT)

	}

	return КукиJWT, КукиИдКлиента
}
...
// ПрочитатьТокенКлиета - распарсим jwt получим ИдКлиента если есть, иначе возвращается пустая строка

func ПрочитатьТокенКлиета(req *http.Request) (ТокенКлиента, string) {

	JWT := req.Header.Get("X-UAT")
	Инфо(" читаем заголвоки X-UAT:%+v \n", JWT)
	...
	if JWT != "" {
		...
		if ДанныеКлиента, ok := token.Claims.(jwt.MapClaims); ok {
			return ТокенКлиента{
				...
				// Роль:      ДанныеКлиента["role"].([]string), // не передаются на клиента
				// Права:     ДанныеКлиента["access"].([]string), // не передаются на клиента
				Истекает: ДанныеКлиента["expi"].(time.Time),
				Создан:   ДанныеКлиента["iat"].(time.Time),
			}, JWT
		} else {
			return ТокенКлиента{}, JWT
		}
...
func ПолучитьУидКЛиента(req *http.Request) uuid.UUID {
	...
	if cookieJWT, err := req.Cookie("jwt"); err != nil {
		...
		Инфо(" Сохраняем данные JWT для идентификации %+v ? мнее кажеться тут это лишнее\n", cookieJWT)
...
func ПолучитьСрезМаршрута(путь string) []string {
	путь = strings.Trim(путь, "/")
	срезМаршрута := strings.Split(путь, "/")
	if len(срезМаршрута) == 0 || (len(срезМаршрута) > 0 && срезМаршрута[0] == "") {
		срезМаршрута[0] = "главная"
		Инфо(" Пустой маршрут, добавляем в маршруты обработку по умолчанию.... %+s \n", срезМаршрута[0])
	}
	return срезМаршрута
}
...
func ОтправитьЗапросВОбработку(ЗапросОтКлиента *http.Request) (Сообщение, error) {
	...
	Инфо("ОтправитьЗапросВОбработку - ЗапросОтКлиента  \n")
	var ИдКлиента uuid.UUID
	var JWT string
	// распарсим jwt получим ИдКлиента если есть, иначе возвращается пустая строка
	...
	// }
	JWT, ИдКлиента = ПроверитьДанныеАаутентификации(ЗапросОтКлиента)
	// запрос.УИДЗапроса = УИДЗапроса(&ИдКлиента, []byte(ЗапросОтКлиента.URL.Path))

	if ИдКлиента == uuid.Nil {
		Инфо("ИдКлиента == uuid.Nil \n")
		ИдКлиента = uuid.New()
	}
	...
	запрос := Запрос{
		...
		КартаМаршрута:  ПолучитьСрезМаршрута(ЗапросОтКлиента.URL.Path),
		Форма:          nil,
		Файл:           "",
	...
	Инфо("типДанных %+v;  ЗапросОтКлиента.Method=AJAXPost %+v \n", типДанных, ЗапросОтКлиента.Method == "AJAXPost")

	if ЗапросОтКлиента.Method == http.MethodPost || ЗапросОтКлиента.Method == "AJAXPost" {
		strings.Split(типДанных, ";")

		if strings.HasPrefix(типДанных, "multipart/form-data") {
			Инфо("нужно реализовать декодирование, я так понимаю тут передаются файлы через форму %+v \n", "multipart/form-data")
			ЗапросОтКлиента.ParseMultipartForm(ЗапросОтКлиента.ContentLength)
		}
		ЗапросОтКлиента.ParseForm()
		Инфо(" ЗапросОтКлиента %+v \n", ЗапросОтКлиента)
		if ЗапросОтКлиента.Form != nil {
			запрос.Форма = ЗапросОтКлиента.Form
		} else {
			запрос.СоставнаяФорма = ЗапросОтКлиента.MultipartForm
		}

		Инфо(" Форма %+v ; \n СоставнаяФорма %+v \n", запрос.Форма, запрос.СоставнаяФорма)

		if СпособВставки, естьСпособОбновления := запрос.Форма["updateMethod"]; естьСпособОбновления {
			запрос.СпособВставки = СпособВставки[0]
		} else {
			запрос.СпособВставки = "append"
		}
	...
	case "AJAX":
		...
		// Инфо(" Запрос отправлен методом  %+v , удалим первый элемент пути, предполагаем что т.к. запрос отправлен ajax то нужен внутрений блок данных, начиная со второго \n", ЗапросОтКлиента.Method )
	...
	Инфо(" Отправляем сообщение в SynQuic  %+v  потокСессии  %+v \n", сообщение, потокСессии.Поток.StreamID())
	ОтправитьСообщение(потокСессии.Поток, сообщение)

	// Получаем ответ от SynQuic
	меткаВремени := time.Now().String()
	Инфо("  %+v  Получаем ответ от SynQuic поток id %+v \n", меткаВремени, потокСессии.Поток.StreamID())
	ответ := ЧитатьСообщение(потокСессии.Поток, "ОтправитьЗапросВОбработку "+меткаВремени)

	// возвращаем поток в очередь
	Инфо(" %+v  возвращаем поток %+v  SynQuic в очередь;  ответ SynQuic %+v \n ", меткаВремени, потокСессии.Поток.StreamID(), ответ)
	ВернутьSynQuicПотокВочередь(потокСессии)
