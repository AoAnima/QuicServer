...
func КодироватьСообщениеОтвет(сообщение Сообщение) []byte {
	...
	case AjaxHTML:
		...
		// Инфо(" AjaxHTML нужно кодировать поле AjaxHTML, пока что кодируются Данные 111 %+s \n", сообщение.ОтветКлиенту)
		// ответ, err := Json(сообщение.ОтветКлиенту.HTML)
		ответ, err := Json(сообщение.ОтветКлиенту.AjaxHTML)
		if err != nil {
			Ошибка("  %+v \n", err)
		}
		Инфо("AjaxHTML %+v \n", ответ)
		...
		return ответ
	case AjaxJSON:
		Инфо(" AjaxJSON нужно кодировать поле  JSON, пока что колируются Данные  \n")
		ответ, err := Json(сообщение.ОтветКлиенту.JSON)
		if err != nil {
			Ошибка("  %+v \n", err)
		}
		return ответ
