...
/*
Найти монстроузная функция, нужно рефакторить
...
*/
func (таблица *Таблица) Найти(ключ string, значение string) (map[string]Данные, ОшибкаБазы) {
	var документы = map[string]Данные{}
	...
	if ключ == string(таблица.ПервичныйКлюч) {
		...
		if _, естьИндексы := таблица.Индексы.КартаИндексов[ПутьИндекса(ключ)]; естьИндексы {
			...
			найденныеКлючи := make(map[string]struct{})
		...
		*/
		if _, естьУникальныеИндексы := таблица.УникальныеИндексы.КартаИндексов[ПутьИндекса(ключ)]; естьУникальныеИндексы {
			...
			if ошибкаБазы.Код == Ок {
				...
				ошибкаПолучениДокумента := таблица.База.ТранзакцияЧтение(func(трзБаза *Транзакция) ОшибкаБазы {
					Инфо(" таблица.База.ТранзакцияЧтение %+v \n", ПервичныйКлюч(ключИндекса))
					// var ошибка ОшибкаБазы
					_, ошибка := трзБаза.Получить(ПервичныйКлюч(ключИндекса), &документ)
		...
		if len(документы) > 0 {
			...
			// если ключа нет в индекса, иkb уникальных индексах, то будем искать по всему документы
			if !естьУникальныеИндексы || !естьИндексы {
				var данные map[string]Данные
				ошибкаПолучениДокумента := таблица.База.ТранзакцияЧтение(func(трзБаза *Транзакция) ОшибкаБазы {
					var ошибка ОшибкаБазы
			...
			// Документы не найден.. нужно как то проверить что
