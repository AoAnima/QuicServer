...
/*
Найти - возвращает карту map[string]Документ где ключём явяется искомое значение ключа переданное в функцию
[монстроузная функция, нужно рефакторить]
Пытается найти просто по ключу в базе, если ключ не совпадает с первичным ключём таблицы, то ищет  парралельно в индексах, в неуникальном и уникальном, если ни там ни там нет ключа поиска, то ищет по всем документам итерируясь и просматривая значения по пути ключа.
