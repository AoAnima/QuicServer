...
func обработчикСообщенийHTTPсервера(сообщение Сообщение) (Сообщение, error) {
	...
	if err != nil {
		...
		if ЕстьОчередиЗапросов {
			...
			// Всё асинхронные запросы отработали, начнём обрабатывать последовательную очередь сервисов. Ну типа ждём их завершения , вдруг резултат из этих обработчиков нужны для получения даных из последовательных Сервисов... ЧТо не логично, т.к. НА то они и ассинхронных чтобы от них ничего не зависило ? Вероятно не обязательно ждать завершения их работы
...
/*
	 ...
	 Наверное нужно каждому шаблону делать сооветсвие сервисов,
	 например :
...
/*
	Док 15.04.2024:
		 В БАзе должен храниться объект, представляющий собоей набор/очередь сервсиов и обработчиков для получения данных, для конкретного url и прав доступа, к примеру
		 url рабочийСтол/настройки
		 будет иметь две разных группы обработчиков в зависимост от прав доступа:
		 роль: клиент
...
func ПолучитьОчередьСервисовИзБД(сообщение *Сообщение) (ОчередьОбработчиковЗапроса, error) {
	...
	сообщение.Запрос.Действие = "получитьОбработчикиДляЗапроса"

	очередь.Добавить(ИмяСервиса("МенеджерОбработчиков"))

	ответМенеджераОбработчиков, err := ОбработатьОчередь(очередь, *сообщение)
	if err != nil {
		Ошибка("  %+v \n", err)
	}
	планОбработки := ответМенеджераОбработчиков.Ответ["МенеджерОбработчиков"]
	сообщение.Запрос.Действие = "" // очистим чтобы сервисы не приглючили
	// обработчикиЗапроса.Данные
	Инфо("планОбработки %+v \n", планОбработки)
	...
	return ОчередьОбработчиковЗапроса{
		// Очередь:            маршрут,
		ПаралельнаяОчередьОбработчиков: make([]string, 0),
	}, nil
}

// ПостроитьМаршрут
/*
Оправляет азпрос в базу даных получает очередь обработчиков для запроса
...
func ПостроитьМаршрут(Сообщение *Сообщение) error {
	...
	if ОчередьМаршрутов.ОчередьОбработчиков == nil && ОчередьМаршрутов.ПаралельнаяОчередьОбработчиков == nil {
		Инфо("  %+v ??\n", "404")
	...
	go func() {
		for _, маршрут := range ОчередьМаршрутов.ОчередьОбработчиков {
	...
	go func() {
		...
		for _, маршрутПаралельногоСервиса := range ОчередьМаршрутов.ПаралельнаяОчередьОбработчиков {
