...
func ПолучитьОчередьСервисовИзБД(сообщение *Сообщение) (ОчередьОбработчиковЗапроса, error) {
	...
	сообщениеСОбработчиками, err := ОбработатьОчередь(очередь, *сообщение)
	if err != nil {
		Ошибка("  %+v \n", err)
	}
	обработчикиЗапроса := сообщениеСОбработчиками.Ответ["МенеджерОбработчиков"]
	...
	очередь := ОчередьОбработчиковЗапроса{
		Очередь:            маршрут,
