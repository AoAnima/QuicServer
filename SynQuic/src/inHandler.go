...
func ОбработатьОчередь(очередьЗапросов *Очередь, сообщение Сообщение) (Сообщение, error) {
	...
	for !очередьЗапросов.Пусто() {
		...
		if err != nil {
			Ошибка("НЕ УДАЛОСЬ ПОЛУЧИТЬ ПОТОК ЛИ СОЗДАТЬ НОВЫЙ потокСессии  %+v  err %+v \n", потокСессии, err)
			return сообщение, err
		}
...
func ПолучитьОчередьСервисовИзБД(сообщениеЗапрос *Сообщение) (ОчередностьСервисовИзБД, error) {
	...
	2.Если нет, то:
		2.1. Если url.path содержит 1 элемент пути то по нему и сопоставляем обработчики из реистратора сервисов
		2.2 если более одного пути то обрабатываем запрос в обратном порядке, вначале второй эломент , потом первый, и пока предолагаем что 1 элемент это основной шаблон, а второй путь это вложенная страница subcontent
	*/

	// это уже сделано в httpServer
	...
	// if len(маршрут) == 0 || (len(маршрут) > 0 && маршрут[0] == "") {
	// 	Инфо(" Пустой маршрут, добавляем в маршруты обработку по умолчанию.... \n")
	// 	if сообщениеЗапрос.JWT != "" {   // не требуется , делаем идентификацию ещё на входе в сервис, при получениии HTTP сообщениея  ИдентификацияКлиента
	...
	*/
	...
	if len(сообщениеЗапрос.Запрос.КартаМаршрута) > 1 {
		картаМаршрута := сообщениеЗапрос.Запрос.КартаМаршрута
		for i := len(картаМаршрута) - 1; i >= 0; i-- {
			маршрут = append(маршрут, картаМаршрута[i])
		}
	} else if len(сообщениеЗапрос.Запрос.КартаМаршрута) == 1 {
		маршрут = append(маршрут, сообщениеЗапрос.Запрос.КартаМаршрута[0])
	...
	// Инфо("  %+v \n", маршрут)
	// if маршрут[0] == "/" {
	// 	// main базовый шаблон главной страницы
		
	// } else {
	
	// 	сообщениеЗапрос.Запрос.ИмяБазовогоШаблона = ИмяШаблона(сообщениеЗапрос.Запрос.КартаМаршрута[0])
	// 	// сообщениеЗапрос.Запрос.ИмяБазовогоШаблона = ИмяШаблона(маршрут[0])
	// }

	имяСервиса, ок := КартаОбработчиков[Маршрут(маршрут[0])]
	if ок {
		шаблонизатор := КартаДанныхШаблона{
			ИмяБазовогоШаблона: сообщениеЗапрос.Запрос.ИмяБазовогоШаблона,
			Данные: map[ИмяСервиса]interface{}{
				имяСервиса: nil,
			},
		}

		сообщениеЗапрос.Запрос.Шаблонизатор = make(map[ИмяШаблона]КартаДанныхШаблона)
