...
var клиенты = make(map[[16]byte]map[Уид]Запрос)
...
func ОтправитьЗапросВОбработку(ЗапросОтКлиента *http.Request) (Сообщение, error) {
	...
	if _, ok := клиенты[сообщение.ИдКлиента]; !ok {
		клиенты[сообщение.ИдКлиента] = map[Уид]Запрос{}
	}
	клиенты[сообщение.ИдКлиента][запрос.УИДЗапроса] = запрос
	// клиенты[ЗапросОтКлиента.ИдКлиента][ЗапросОтКлиента.УИДЗапроса] = ЗапросОтКлиента
	мьютекс.Unlock()
	Инфо("ОтправитьЗапросВОбработку  клиенты %+v \n", клиенты)

	// Отправляем данные в SynQuic
	// ОтправитьСообщение(SynQuicПоток, Сообщение)
