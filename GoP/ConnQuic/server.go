...
var СессииСервисов КартаСервисов = make(map[Сервис]*КартаСессий)
...
func РегистрацияСервиса(сессия *quic.Connection, поток *quic.Stream, сообщение *Сообщение) {
	...
	if картаСессий, ok := СессииСервисов[сообщение.Сервис]; ok {
		...
		очередьПотоков.Добавить(*поток)
		СессииСервисов[сообщение.Сервис] = &КартаСессий{
