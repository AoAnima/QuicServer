...
func ЗапуститьСервер(Адрес string, обработчикСообщений func(поток *quic.Stream, сообщение Сообщение)) {
	...
	for {
		...
		go ОбработчикЗапросовНаОткрытиеВходящегоПотока(сессия)
	}
}

func ОбработчикЗапросовНаОткрытиеВходящегоПотока(сессия quic.Connection) {
	for {
		поток, err := сессия.AcceptStream(context.Background())
		if err != nil {
			Ошибка("  %+v \n", err)
		}
		Инфо("ОбработчикЗапросовНаОткрытиеВходящегоПотока ? пришёл запрос от клиента на открытие нового потока, клиенту некуда слать запросы....   %+v %+v \n", поток, сессия)
...
var МьютексАктивныхСессий sync.Mutex
var АктивныеСессииСервисов КартаСервисов = make(map[Сервис]*КартаСессий)
...
func РегистрацияСервиса(сессия *quic.Connection, поток *quic.Stream, сообщение *Сообщение) {
	...
	if картаСессий, ok := АктивныеСессииСервисов[сообщение.Сервис]; ok {
		...
		очередьПотоков.Добавить(поток)
		АктивныеСессииСервисов[сообщение.Сервис] = &КартаСессий{
...
func ОтправитьВПоток(указательНаПоток *quic.Stream, сообщение []byte) error {
	поток := *указательНаПоток
...
func ОтправитьСообщение(сервис Сервис, сообщение Сообщение) {
	поток, err := ПотокДляСервиса(сервис)
	if err != nil {
		Ошибка("  %+v \n", err)
		return
	}
	...
	err = ОтправитьВПоток(поток, СообщениеВОтправку)
	if err != nil {
		Ошибка("  %+v \n", err)
	}
	ВернутьПотокВОчередь(сервис, поток)
}

func ВернутьПотокВОчередь(сервис Сервис, поток *quic.Stream) {
	МьютексАктивныхСессий.Lock()
	defer МьютексАктивныхСессий.Unlock()
	СессииСервиса, ok := АктивныеСессииСервисов[сервис]
	if ok {
		СессииСервиса.ОчередьПотоков.Вернуть(поток)
	} else {
		Ошибка(" в АктивныеСессии %+v нет сервиса: %+v поток не добавлен в очередь , мжет нужно его закрыть ? Или проверить что случилось с сервисом\n", АктивныеСессииСервисов, сервис)
	}

}
func ПотокДляСервиса(сервис Сервис) (*quic.Stream, error) {
	МьютексАктивныхСессий.Lock()
	defer МьютексАктивныхСессий.Unlock()
	СессииСервиса, ok := АктивныеСессииСервисов[сервис]
	if ok {
		if поток := СессииСервиса.ОчередьПотоков.Взять(); поток != nil {
			return поток, nil
		} else {
			return nil, errors.New("нет свободных потоков для сервиса " + string(сервис))
		}

	} else {
		return nil, errors.New("нет заргеитсрированных сервисов " + string(сервис))
	}
}
