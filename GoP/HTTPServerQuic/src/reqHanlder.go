...
var клиенты = make(map[[16]byte]КартаКлиентов)
...
func ОтправитьЗапросВОбработку(ЗапросОтКлиента *http.Request) (Сообщение, error) {
	...
	запрос.УИДЗапроса = УИДЗапроса(&сообщение.ИдКлиента, []byte(ЗапросОтКлиента.URL.Path))
	сообщение.УИДСообщения = запрос.УИДЗапроса
	БлокКлиентов.Lock()
	...
	Инфо(" Отправляем сообщение в SynQuic  %+v \n", сообщение)
...
func ОчисткаКартыКЛиентов() {
	Инфо("ОчисткаКартыКЛиентов проверяет как давно была последняя активность клиента, если более5 минут назад то удалёт клинета из карты %+v \n")

	for ИдКлиента, КартаЗапросов := range клиенты {
		if len(КартаЗапросов.Запросы) == 0 {
			if time.Since(КартаЗапросов.ПоследняяАктивность) > 5*time.Minute {
				БлокКлиентов.Lock()
