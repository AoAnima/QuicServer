...
func обработчикСообщенийHTTPсервера(поток quic.Stream, сообщение Сообщение) {
	go СохранитьСообщение(&сообщение)
	Инфо(" cообщение %+v %T \n", сообщение, сообщение)
	...
	if err != nil {
		...
		if ЕстьОчередиЗапросов {
			...
			if len(очередьЗапросов.ПаралельнаяОчередь) > 0 {
				ОбработатьПаралельныеСервисы(&очередьЗапросов, сообщение)
			}
		}
...
func ОбработатьПаралельныеСервисы(паралельныеСервисы *СтруктураКартыЗапросов, сообщение Сообщение) {
	for имяПаралельногоСервиса := range паралельныеСервисы.ПаралельнаяОчередь {
		if картаСервиса, есть := АктивныеСессии[имяПаралельногоСервиса]; есть {

			for _, сессия := range картаСервиса {
				if !сессия.ОчередьПотоков.Пусто() {
					поток := сессия.ОчередьПотоков.Взять()
					сообщениеБин, err := Кодировать(сообщение)
					if err != nil {
						Ошибка("  %+v \n", err)
					}
					поток.Write(сообщениеБин)
