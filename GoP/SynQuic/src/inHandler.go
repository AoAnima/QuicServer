...
<<<<<<< HEAD
// Обрабатывает входящие сообщения , в принципе все, строит маршрут куда отправить запросы для генерации отвта на сообщение , и отправляет в соотвествующие сервисы, в конце отправляет ответ в поток, поток зарезервирован и ожидает ответа на другой стороне.
func обработчикВходящихСообщений(поток quic.Stream, сообщение Сообщение) {
=======
// Обрабатывает только запросы полученный от сервисов, в ответ ничего не отправляет
func обработчикВходящихСообщений(потокДляОтправкиОтвета *quic.Stream, сообщение Сообщение) {
...
<<<<<<< HEAD
	//ДОДЕЛАТЬ: АНАЛИЗИРУЕМ сообщение, находим в АктивныеСессии нужный сервис, отправляем в поток из очереди, сообщение
	//DEBUG:
	/* ДОК: выдлаоаывлао;*/

	СообщениеОтвет := ПолучитьДанные(сообщение)

	Инфо(" СообщениеОтвет %+v %T \n", СообщениеОтвет, СообщениеОтвет)
	Инфо(" cообщение %+v %T \n", сообщение, сообщение)

	Ответ, err := Кодировать(сообщение)
=======
	// Обрабатываем сообщение, строим маршрут для отправки в сервисы, отправляем в сервисы, получаем ответы, рендерим
	СообщениеОтвет := ОбработатьСообщение(сообщение)
...
>>>>>>> 749006ec09c54c1e21404de823aefea1a35f2753
	...
	// ОтправитьСообщение(поток, Ответ)
	// отправляем ответ обработ
	err = ОтправитьВПоток(потокДляОтправкиОтвета, Ответ)
	if err != nil {
		Ошибка("  %+v \n", err)
	}
}
...
var КартаЗапросов = make(map[Уид]struct {
	Очередь          *Очередь
	Паралельно       map[Сервис]ОтветПолучен // [сервис]ОтветПолучен
...
func ОбработатьСообщение(Сообщение Сообщение) *Сообщение {
	...
	*/
	...
	if ЕстьОчередиЗапросов {
		...
		_, естьОжиданиеОтвета := очередьЗапросов.Паралельно[Сервис(Сообщение.Сервис)]
		if естьОжиданиеОтвета { // len(очередьЗапросов.Паралельно) > 0 &&
			 ...
			 */
			очередьЗапросовНеПуста := !очередьЗапросов.Очередь.Пусто()
			if очередьЗапросовНеПуста {
				...
				if сервис != "" && err == nil {
					...
					Сообщение.Ответ[сервис] = ОтветСервиса{
			...
			if !очередьЗапросовНеПуста && len(очередьЗапросов.Паралельно) == 0 {
				БлокКартаЗапросов.Lock()
				финальныйСервис := очередьЗапросов.ФинальныеСервисы.Далее()
				БлокКартаЗапросов.Unlock()

				// если есть финальные сервис, то соберём все ответы в кучу,  отправим всё в финальные сервисы, для рендера и упорядочивания данных
				if финальныйСервис != nil {
					// перенесём все ответы из паралельных сервисов в ответное сообщение
					...
					// если финальный сервисы пусты, то возвращаем сообщение клиенту с ответом
					return &Сообщение
				}
			}
...
func ПолучитьСледующийСервис(УИДСообщения *Уид) (Сервис, error) {
	БлокКартаЗапросов.Lock()
	...
	if очередьСервисов, ok := КартаЗапросов[*УИДСообщения]; ok {
		...
		if СледующийСервис != nil {
			...
			return СледующийСервис.(Сервис), nil
		} else {
...
func ПостроитьМаршрут(Сообщение *Сообщение) error {
	...
	ПаралельныеСервисы := make(map[Сервис]ОтветПолучен)
	for _, сервис := range ОчередьМаршрутов.Паралельно {
		ПаралельныеСервисы[Сервис(сервис)] = false
	}
	...
	КартаЗапросов[Сообщение.Запрос.УИДЗапроса] = struct {
		Очередь          *Очередь
		Паралельно       map[Сервис]ОтветПолучен
...
func ОтправитьЗапросВСервис(сервис Сервис, cообщение Сообщение) {

	if Сервис, есть := АктивныеСессииСервисов[сервис]; есть {
		Инфо(" Сервис %+v \n", Сервис)
		// Сервис.КаналСообщения читается в функции ОтправитьСообщениеВСервис()
		cообщениеЗакодированное, err := Кодировать(cообщение)
		if err != nil {
			Ошибка("  %+v \n", err)
		}
		МьютексАктивныхСессий.Lock()
		потокСервиса := *Сервис.ОчередьПотоков.Взять()
		if потокСервиса == nil {
			потокСервиса = *Сервис.СоздатьНовыйПоток()
		}

		отправленноБайт, err := потокСервиса.Write(cообщениеЗакодированное)
		if err != nil {
			Ошибка("  %+v отправленноБайт %+v \n", err, отправленноБайт)
		}
		Сервис.ОчередьПотоков.Вернуть(&потокСервиса)
...
var КартаЗапросов = make(map[Уид]struct {
	Очередь            *Очередь
	ПаралельнаяОчередь map[Сервис]ОтветПолучен // [сервис]ОтветПолучен
	ФинальныеСервисы   *Очередь                //сервисы которые выполняются после получения всех ответов от Паралельных и последовательных, Сюда добавляем такие сервисы как Рендер - который рендерит ответ, в html или json структуру заданного формата, и Сервис от которого пришёл первый зпрос
})
...
func ПолучитьДанные(Сообщение Сообщение) *Сообщение {
	...
	*/
	...
	if ЕстьОчередиЗапросов {
		...
		// Вначале проверим не пришёл ли ответ от Сервиса обрабатывающего паралельный запрос, если очерель паралельнызх запросов не пуста и Такой сервис есть в Очереди Ожидающих ответов, то сохраним ответ, и продолжим ожидание сообщений, если же очередь паралельных запросв пуста или в очереди нет Сервиса, то перейдём к последовательным запросам
		Инфо(" очередьЗапросов %+v \n", очередьЗапросов)
		_, естьОжиданиеОтвета := очередьЗапросов.ПаралельнаяОчередь[Сервис(Сообщение.Сервис)]
		if естьОжиданиеОтвета { // len(очередьЗапросов.ПаралельнаяОчередь) > 0 &&
			...
			if неПусто := !очередьЗапросов.Очередь.Пусто(); неПусто {
				...
				if сервис != "" && err == nil {
					...
					Инфо(" Запрос.Ответ проверка до изменения ответов %+v \n", Сообщение.Ответ)
					Сообщение.Ответ[Сервис(сервис)] = ОтветСервиса{
						Сервис:          Сервис(сервис),
						УИДЗапроса:      string(Сообщение.Запрос.УИДЗапроса), // потому что сервис при ответе в УИДСообщения напишет своё значение
						Данные:          nil,
						ЗапросОбработан: false,
			...
			// Проверим если обе очереди пусты, то объеденим ответы от всех сервисов и отправим в финальные сервисы
			if очередьЗапросов.Очередь.Пусто() && len(очередьЗапросов.ПаралельнаяОчередь) == 0 {
				БлокКартаЗапросов.RLock()
...
func ПостроитьМаршрут(Сообщение *Сообщение) error {
	...
	for _, имяСервиса := range ОчередьМаршрутов.Очередь {
		// проверим есть ли обработчик для маршрута в карте маршуртизатора, если сесть добавим в очередь иначе 404

		if _, есть := АктивныеСессии[Сервис(имяСервиса)]; есть {
			Инфо(" добавляем %+v \n", имяСервиса)
			ОчередьСеревисов.Добавить(имяСервиса)
		} else {
	...
	ПаралельныеСервисы := make(map[Сервис]ОтветПолучен)
	for _, имяПаралельногоСервиса := range ОчередьМаршрутов.ПаралельнаяОчередь {
		ПаралельныеСервисы[Сервис(имяПаралельногоСервиса)] = false
	}
	ОчередьФинальныеСервисы := НоваяОчередь()
	...
	КартаЗапросов[Сообщение.Запрос.УИДЗапроса] = struct {
		Очередь            *Очередь
		ПаралельнаяОчередь map[Сервис]ОтветПолучен
		ФинальныеСервисы   *Очередь
	}{
		Очередь:            ОчередьСеревисов,
