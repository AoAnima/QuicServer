...
func ПрочитатьСообщение(conn *fastws.Conn, каналЗапросов chan Запрос) {
	...
	for {
		...
		if сообщение != nil {
			...
			int, err := клиенты[ответ.Ид].ResponseWriter.Write(сообщение)
			Инфо(" int= %+v err %+v \n", int, err)
			
			close(клиенты[ответ.Ид].Req.Response.Request.Context().Done())

			if f, ok := клиенты[ответ.Ид].ResponseWriter.(http.Flusher); ok {
				Инфо(" ok= %+v f %+v \n", ok, f)
				клиенты[ответ.Ид].ResponseWriter.WriteHeader(http.StatusOK)
