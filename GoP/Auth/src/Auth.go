...
func ОбработчикЗапросовСервера(поток quic.Stream, сообщение Сообщение) {
	...
	case "checkLogin":
		ok, err = ПроверитьЛогин(&сообщение)
...
func ПроверитьЛогин(сообщение *Сообщение) (bool, error) {
	Инфо("ПроверитьЛогин , нужно проверить логин на свободу \n")

	return true, nil
}
func ПроверитьEmail(сообщение *Сообщение) (bool, error) {
...
func Регистрация(сообщение *Сообщение) (bool, error) {
	...
	if len(форма) > 0 {
		...
		свободен, err := ПроверитьЛогин(сообщение)
		if !свободен && err != nil {
			ответ.Данные = map[string]string{
		...
		emailСвободен, err := ПроверитьEmail(сообщение)
