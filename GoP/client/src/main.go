...
func clinet() {
	...
	tlsconfig, err := КлиентскийТлсКонфиг()
	if err != nil {
		log.Fatal(err)
	}
	// Подключитесь к серверу
	session, err := quic.DialAddr(ctx, "localhost:4242", tlsconfig, config)
	if err != nil {
		log.Fatal(err)
	}
...
func server() {
	...
	config, err := КлиентскийТлсКонфиг()
	if err != nil {
		log.Println(err)
	}
	// Создайте сервер
