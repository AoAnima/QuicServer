...
func АнализЗапроса(Запрос *connector.Сообщение) {
	...
	*/
	...
	// TODO: Перед отправко й вполучить маршрут нужно проанализировать, КартаЗапросов если тут уже есть УИДСообщения то не вызываем эту функцию а получаем следующий сервис

	ПолучитьМаршрут(Запрос)
	сервис := ПолучитьСледующийСервис(&Запрос.УИДСообщения)
	if сервис != "" {
...
func ПолучитьСледующийСервис(УИДСообщения *connector.УидЗапроса) string {
	БлокКартаЗапросов.Lock()
	defer БлокКартаЗапросов.Unlock()
	if сервис, ok := КартаЗапросов[*УИДСообщения]; ok {
		return сервис.Далее().(string)
	}
	return ""

}

func ПолучитьМаршрут(Запрос *connector.Сообщение) {
	...
	ОчередьСеревисов.Добавить(маршрут[0])
