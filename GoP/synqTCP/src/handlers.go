...
func обработчикИсходящихСоединений(клиент net.Conn, каналОтвета <-chan ОтветКлиенту) { //, данныеДляОтвета chan []byte
...
func РукопожатиеИсходящегоКанала(клиент net.Conn) {
	...
	for {
		...
		if длинаСообщенияФикс == [4]byte{240, 159, 164, 157} { //"🤝"
			...
			КаналыИсходящихСообщений[string(ИмяСервиса)] = make(chan ОтветКлиенту, 10)
			continue
		}
	}
}
