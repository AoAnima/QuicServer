...
//		},
//	}
func (о *Отпечаток) ЧитатьКаналСообщений() {
...
func (о *Отпечаток) ПолучитьСоединение() {
...
func РегистрацияСервиса(отпечатокСервиса *Отпечаток, Клиент net.Conn) {
...
func АнализЗапроса(Запрос *ЗапросКлиента) {
	...
	параметрыЗапроса, err := url.Parse(Запрос.Запрос.Строка)
...
func ОтправитЗапросВСервис(параметрыЗапроса *url.URL, СтрокаЗапроса *ЗапросКлиента) {
	...
	if Сервис, есть := Маршрутизатор[маршрут[0]]; есть {
		Инфо(" Сервис %+v \n", Сервис)
		Сервис.КаналСообщения <- СтрокаЗапроса
	} else {
