...
func Обработать(запрос connector.ЗапросКлиента, ошибкаДекодирования error) ([]byte, error) {
...
func ДекодироватьПакет(пакет []byte) (connector.ЗапросКлиента, error) {
	...
	var ДанныеВОбработку = connector.ЗапросКлиента{}

	err := jsoniter.Unmarshal(пакет, &ДанныеВОбработку)
	if err != nil {
		Ошибка("  %+v \n", err)
		return connector.ЗапросКлиента{}, err
	}
...
func АнализЗапроса(Запрос *connector.ЗапросКлиента) {
...
func СохранитьЗапрос(запрос *connector.ЗапросКлиента) {
