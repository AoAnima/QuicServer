...
type ОчередьОбработчиковЗапроса struct {
	...
	Шаблон                         ИмяШаблона // если данные нужно от рендерить то впишем имя шаблона ?
}
...
type ОтветСервиса struct {
	...
	// AjaxHTML        map[string]ДанныеAjaxHTML // Типа map[id селектор для вставки в HTML]<html для вставки>
	Данные map[string]interface{} // данные в виде структуры какойто
...
type Обработчики struct {
	ОчередьОбработчиков             []Обработчик `json:"очередь_обработчиков,omitempty"` //"очередь_обработчиков":[{"uid":"0x186a7"}],
	АссинхроннаяОчередьОбработчиков []Обработчик `json:"ассинхронная_очередь_обоработчиков,omitempty"`
