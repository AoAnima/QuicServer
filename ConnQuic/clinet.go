...
func (клиент Клиент) Соединиться(
	...
	for сессия == nil {
		...
		if ошибкаСоединения != nil {
			Ошибка(" не удаётся покдлючиться к серверу  %+v сессия %+v \n", ошибкаСоединения.Error(), сессия)
...
func (данныеСессий ДанныеСессии) ЧитатьСообщения(поток quic.Stream, обработчикСообщений func(поток quic.Stream, сообщение Сообщение)) {
	...
	for {
		...
		прочитаноБайт, err := io.ReadFull(поток, сообщениеБинарное)
