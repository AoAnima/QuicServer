...
func (транзакция Транзакция) Измененить(запрос ДанныеЗапроса, ctx context.Context) (map[string][]map[string]string, СтатусБазы) {
	if ctx == nil {
		ctx = context.Background()
	}
	...
	СуществующиеЗаписи := map[string][]map[string]string{}
