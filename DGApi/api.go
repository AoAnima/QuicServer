...
func (база СоединениеСДГраф) Изменить(запрос ДанныеЗапроса) ([]byte, СтатусБазы) {
	...
	if ошибка != nil {
		Ошибка("  %+v запросВБД %+s апиМутации %+v \n", ошибка.Error(), запросВБД)
		for _, мутация := range мутации {
			Инфо(" Мутация %+s \n", мутация.Мутация)
...
func (транзакция *Транзакция) Получить(запрос ДанныеЗапроса) ([]byte, СтатусБазы) {
