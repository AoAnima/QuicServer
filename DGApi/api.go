...
Изменить принимает запрос и выполняет его, поэтому если в запросе КОНКРЕТНО не указан uid вложенного узла , то он не будет обновлен, и если будет передан объект в котром нужно обновить значение вложенного узла, то ббудет добавлен новый узел с новым значением, а старые никуда не денется и будет иметь связь с родительсим объектом. И нужно либо удалить этот объект либо разорвть связь.

Такой подход мжет быть полезен в системах где необходимо хранить всю историю изменений, тогда каждый узел должен хранить дату своего создания.?

Поэтому нужно сделать не забывать в Запросах в базу, указывать uid вдочернего узла и новое значение. Чтобы не создавать новые узлы если это не нужно.
*/
