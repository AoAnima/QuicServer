...
//		}
//	}
func Изменить(запрос ДанныеЗапроса, граф *dgo.Dgraph) (string, СтатусБазы) {
	for {
		ctx := context.Background()
		...
		return результат.String(), СтатусБазы{
			Код: Ок,
		}
	}
}
...
Берёт соединнение из пула, отправляет запрос и возвращает соелинение в пул
*/
func Получить(запрос ДанныеЗапроса, граф *dgo.Dgraph) (string, СтатусБазы) {
	ctx := context.Background()
	транзакция := граф.NewReadOnlyTxn()
	defer транзакция.Discard(ctx)
