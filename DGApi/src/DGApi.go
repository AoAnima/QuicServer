...
func ТестОбработчиков() {
	...
	новыйОбработчик := &КонфигурацияОбработчика{
		Тип:        "Обработчик",
		Маршрут:    "/редакторОчередей",
		Действие:   "добавитьОбработчикВОчередь",
		Обработчик: "добавитьОбработчикВОчередь",
		ПраваДоступа: []ПраваДоступа{
			{
				Тип:   "ПраваДоступа",
				Роль:  []string{"админ"},
				Права: []string{"чтение", "запись"},
			},
		},
		Описание: "обработчик для создания очереди обработчиков",
		Шаблонизатор: []Шаблон{
			{
				Тип:    "Шаблон",
				Код:    Ок,
	...
	данные := ДанныеЗапроса{
		Запрос: `query <Обработчики>($hanlder : string, $path : string, $action : string) {
							handlers as <Обработчик>(func: type(<Обработчик>)) {				
								<обработчик>
								<маршрут>
								<действие>
							}	
							 ...
							 <Обработчикиs>(func: ge(count(handlers), 0)) {		
								count(handlers)													
							}	
							j(func: uid(handlers)) {
								<обработчик>
	...
	данные = ДанныеЗапроса{
		Запрос: `query <Обработчики>($hanlder : string, $path : string, $action : string) {
							<Обработчик>(func: eq(<обработчик>, $hanlder))  {
								uid
								<маршрут> @filter(eq(<маршрут>, $path))
								<действие> @filter(eq(<маршрут>, $action))
							}
							<Обработчикss>(func: has(<обработчик>)){				
								<маршрут>
								<действие>
								<обработчик>
								<права>{
