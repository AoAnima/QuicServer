...
func ДобавитьОбработчик(поток quic.Stream, сообщение Сообщение) {
	...
	данныеНовогоОбработчика := СобратьДанныеОбработчика(сообщение.Запрос.Форма)
	рендерЗапроса(данныеНовогоОбработчика)
	новыйОбработчик, _, статус := НовыйОбработчикМаршрута(сообщение.Запрос.Форма)
	if статус.Код != Ок {
...
добавляем новый маршрут:
{
	...
	/*ДОК:
		...
		----Прийдётся отправлять два запроса в транзакции, первый запрос на получение uid ролей. прав. и обработчиков, второй на вставку в базу плана обработчки маршрута но уже с полученными uid.--- не корректно

		Можно попробовать собирать запросы прям тут вместе с данными.


		Генерируем мульт запрос, который будет заправшивать uid для всех данных в виде

		тоесть для каждого параметра свой запрос
		 role_1 as var(func: type(<Роль>)) @filter(eq(<код.роли>, 1))

		 ЗАтем эти переменные будут подставлятся в мутацию
	...
	query{
			<роль_%d> as var(func: type(<Роль>)) @filter(eq(<код.роли>, %d))....
			<права_%d> as var(func: type(<Права>)) @filter(eq(<код.прав>, %d))....
			<обработчик_%d> as var(func: type(<Права>)) @filter(eq(<код.прав>, %d))....
...
// запросРолей := `<роли>(func: type(<Роль>)) @filter(%s) {
// 	uid
// 	<имя.роли>
// 	<код.роли>
// }`
...
// 1. Получаем UID всех добавляемых обработчиков, ролей и прав.
// 2. Проверяем есть ли обработчик с заданым маршрутом - если в бд есть объекта с таким маршрутом, то добавляем новый обхект со всеми обработчиками, иначе эта мутация не выполняется
// 3. в остальных мутациях прверяем маргрут + 1 роль, если пусто то мутируем, иначе нет. И так как это НовыйОбработчикМаршрута то - НЕ ОБОВЛЯЕМ

func НовыйОбработчикМаршрута(форма map[string][]string) (мутацияДобавления []byte, запросПеременных []byte, статус СтатусСервиса) {
	маршрут, статусМаршрута := ЗначениеФормы("маршрут", форма)
	if статусМаршрута.Код != Ок {
		Ошибка(" статус получения маршрут  %+v \n", статусМаршрута)
	}
	комманда, статусКомманды := ЗначениеФормы("имяКоманды", форма)
	if статусКомманды.Код != Ок {
		Ошибка(" статус получения имяКоманды  %+v \n", статусКомманды)
	}

	описание, статусОписания := ЗначениеФормы("описание", форма)
	if статусОписания.Код != Ок {
		Ошибка(" статус получения описание  %+v \n", статусОписания)
	}
	данные := СгрупироватьДанныеФормы(форма)

	имяШаблона, статусШаблон := ЗначениеФормы("шаблон", форма)
	if статусШаблон.Код != Ок {
		Ошибка(" статус получения описания  %+v \n", статусШаблон)
	}

	поискРолей := map[string]string{}
	поискПрав := map[string]string{}
	поискОбработчиков := map[string]string{}
	проверкаДублдикатаМаршрутаДляРоли := map[string]string{}
	// шаблонПоискРолей := `<роль_%d> as var(func: type(<Роль>)) @filter(eq(<код.роли>, %d))`
	// шаблонПоискПрав := `<права_%d> as var(func: type(<Права>)) @filter(eq(<код.прав>, %d))`
	// шаблонПоискПрав := `<обработчик_%d> as var(func: type(<Обработчик>)) @filter(eq(<сервис>, "{{имяСервиса}}") AND eq(<имя_обработчика>,"{{имяОбработчика}}"))`

	// eq(<код.роли>, 2) OR eq(<код.роли>, 3)
	Инфо(" данные%+v \n", данные)

	Мутации := []Мутация{}

	массивОбработчиков := []ПланОбработки{}
	for _, группа := range данные {

		новыйПлан := ПланОбработки{}
		мутация := Мутация{}

		// Если в мапе  есть ключ "роль" то собираем в структуру с правами доступа
		if роль, естьРоль := группа["роль"]; естьРоль {

			поискРолей["<роль_"+роль[0]+">"] = `<роль_` + роль[0] + `> as var(func: type(<Роль>)) @filter(eq(<код.роли>, ` + роль[0] + `))`

			новыйПлан = ПланОбработки{
				Тип:                 "ОбработчикМаршрута",
				ОчередьОбработчиков: []НаборОбработчиков{},
				АссинхроннаяОчередьОбработчиков: []НаборОбработчиков{},
				Права:      []string{},
				ИмяШаблона: имяШаблона,
				Роль:       `uid(<роль_` + роль[0] + `>)`,
			}

			// новыйПлан.Роль = `uid(<роль_` + роль[0] + `>)`

			проверкаДублдикатаМаршрутаДляРоли[`<обработчики_роль_`+роль[0]+`>`] = `<обработчики_роль_` + роль[0] + `> as var(func: eq(<маршрут>, ` + маршрут + `)) @filter(uid(<роль_` + роль[0] + `>))`

			мутация.Условие = `@if(eq(len(<обработчики_роль_` + роль[0] + `>),0))`

			for _, кодПрав := range группа["права_доступа"] {
				поискПрав["<права_"+кодПрав+">"] = `<права_` + кодПрав + `> as var(func: type(<Права>)) @filter(eq(<код.прав>, ` + кодПрав + `))`

				новыйПлан.Права = append(новыйПлан.Права, `uid(<права_`+кодПрав+`>)`)
			}

		} else {
			Ошибка(" в Группе данных  %+v  нет роли для маршрута \n", группа)
		}
		...
		if очередь, естьОчередь := группа["очередь"]; естьОчередь {
			...
			// `<обработчик_%d> as var(func: type(<Обработчик>)) @filter(eq(<сервис>, %s) AND eq(<имя_обработчика>, %s))`
			...
			поискОбработчиков[`<обработчик_`+очередь[0]+`>`] = `<обработчик_` + очередь[0] + `> as var(func: type(<Обработчик>)) @filter(eq(<сервис>, "` + имяСервиса[0] + `") AND eq(<имя_обработчика>,"` + имяОбработчика[0] + `"))`

			очередность := new(int)
			*очередность = номерОчереди
			новыйПлан.ОчередьОбработчиков = append(новыйПлан.ОчередьОбработчиков,
				НаборОбработчиков{
					UID:     `uid(<обработчик_` + очередь[0] + `>)`,
					Очередь: очередность,
				})
		...
		if _, естьАссинхронно := группа["ассинхронно"]; естьАссинхронно {
			...
			поискОбработчиков[`<обработчик_`+имяОбработчика[0]+`>`] = `<обработчик_` + имяОбработчика[0] + `> as var(func: type(<Обработчик>)) @filter(eq(<сервис>, "` + имяСервиса[0] + `") AND eq(<имя_обработчика>,"` + имяОбработчика[0] + `"))`

			новыйПлан.АссинхроннаяОчередьОбработчиков = append(новыйПлан.ОчередьОбработчиков,
				НаборОбработчиков{
					UID: `uid(<обработчик_` + имяОбработчика[0] + `>)`,
				})
		}

		новыйБинарныйПлан, ошибка := Json(новыйПлан)
		if ошибка != nil {
			Ошибка(" ОписаниеОшибки  %+v \n", ошибка)
		}

		мутация.Мутация = новыйБинарныйПлан
		Мутации = append(Мутации, мутация)
		массивОбработчиков = append(массивОбработчиков, новыйПлан)
	}

	// эта структура мутирует когда нет маршрута вообще, без какой либо роли просто eq(<маршрут>,имяМаршрута)
	п := НовыйПланОбработкиМаршрута{
		Тип:         "Обработчики",
		Маршрут:     маршрут,
		Комманда:    комманда,
		Обработчики: массивОбработчиков,
		Описание:    описание,
	}

	мутацияНовогоПлана, ошибка := Json(п)

	if ошибка != nil {
		Ошибка(" статус %+v мутацияПлана %+s \n", статус, мутацияНовогоПлана)

	}
	Инфо(" массивОбработчиков %+v  мутацияПлана %+s Мутации %+v \n", массивОбработчиков, мутацияНовогоПлана, Мутации)

	// роли := strings.Join(поискРолей, "\n")
	// пава:=strings.Join(поискПрав, "\n")
	// обработчики:=strings.Join(поискОбработчиков, "\n")

	return мутацияНовогоПлана, []byte{}, СтатусСервиса{
		Код:   Ок,
		Текст: "Запрос сгенерирован успешно",
	}

	// ФильтрРолей := strings.Join(фильтрРолей, " OR ")
	// запрос := fmt.Sprintf(запросРолей, ФильтрРолей)

	// Инфо(" Сгенерированый запрос %+v \n", запрос)
...
func СгрупироватьДанныеФормы(форма map[string][]string) map[string]map[string][]string {
	...
	for ключ, значение := range форма {
		...
		идПоля := ключ[индексКвадратныхСкобок+1 : len(ключ)-1] // получем то что внутри квадртаных скобк полагая что после квадратных скобок ничего нет... тоест такое ключ[ид][ключ1] не сработает
...
/*
	рендерЗапроса(map[string]interface{}{
			"очередьОбработчиков": очередьОбработчиков,
			"очередьАссинхронныхОбработчиков": очередьАссинхронныхОбработчиков,
			"праваДоступа": праваДоступа,
		})
*/
func рендерЗапроса(данныеОбработчика map[string]interface{}) {
	jet.ПарсингШаблона("templates/request.html", даннеОбработчика)
}
func СобратьДанныеОбработчика(форма map[string][]string) map[string]interface{} { //([]Обработчик, []Обработчик, []ПраваДоступа)
	маршрут, статусМаршрута := ЗначениеФормы("маршрут", форма)
	if статусМаршрута.Код != Ок {
		Ошибка(" статус получения маршрут  %+v \n", статусМаршрута)
	}
