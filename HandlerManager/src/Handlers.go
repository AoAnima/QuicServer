...
func ДобавитьОбработчик(поток quic.Stream, сообщение Сообщение) {
	...
	новыйОбработчик, статус := НовыйОбработчикМаршрута(сообщение.Запрос.Форма)
	if статус.Код != Ок {
...
func НовыйОбработчикМаршрута(форма map[string][]string) (мутацияДобавления []byte, запросПеременных []byte, статус СтатусСервиса) {
	...
	for _, группа := range данные {
		...
		if роль, естьРоль := группа["роль"]; естьРоль {
			...
			// поискРолей = append(поискРолей, fmt.Sprintf(шаблонПоискРолей, кодРоли, кодРоли))
			поискРолей["<роль_"+роль[0]+">"] = `<роль_` + роль[0] + `> as var(func: type(<Роль>)) @filter(eq(<код.роли>, ` + роль[0] + `))`
			// фильтрРолей = append(фильтрРолей, fmt.Sprintf("eq(<код.роли>,"+роль[0]+")"))
			новыйПлан.Роль = `uid(<роль_` + роль[0] + `>)`
			// фильтрРолей = append(фильтрРолей, fmt.Sprintf("eq(<код.роли>, %d)", кодРоли))
			...
			проверкаДублдикатаМаршрутаДляРоли[`<обработчики_роль_` + роль[0] + `>`] := `var(func: eq(<маршрут>, `+маршрут+`)) {  
				<обработчики_роль_` + роль[0] + `>  @filter(uid(<роль_` + роль[0] + `>)){     				
					{
						uid
						<код.роли>
						<имя.роли>
			...
			for _, кодПрав := range группа["права_доступа"] {
				...
				// фильтрПрав = append(фильтрПрав, fmt.Sprintf(шаблонПоискПрав, кодПрав, кодПрав))
		...
		if очередь, естьОчередь := группа["очередь"]; естьОчередь {
			...
			*очередность = номерОчереди
			новыйПлан.ОчередьОбработчиков = append(новыйПлан.ОчередьОбработчиков, НаборОбработчиков{
	...
	мутацияПлана, ошибка := Json(п)

	if ошибка != nil {
		Ошибка(" статус %+v мутацияПлана %+v \n", статус, мутацияПлана)

	}
	Инфо(" массивОбработчиков %+v  мутацияПлана %+s \n", массивОбработчиков, мутацияПлана)
	...
	проверкаДублдиката := `var(func: eq(<маршрут>, `+маршрут+`)) {  
								<обработчики_роль_>  @filter(uid(<роль_>)){     				
									{
										uid
										<код.роли>
										<имя.роли>
	...
	return мутацияПлана, СтатусСервиса{
		Код:   Ок,
