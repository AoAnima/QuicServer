...
var Маршруты = map[string]func(quic.Stream, Сообщение){
	...
	"получитьСписокПрав":            получитьСписокПрав,
	"получитьСписокРолей":           получитьСписокРолей,
	...
	"настройки":                     ПолучитьСписокОчередей,
	"получитьОбработчикиДляЗапроса": ПолучитьОбработчикиДляЗапроса,
}
