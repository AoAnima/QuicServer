...
/*
Получить
Если е передан второй аргумент , в который нужно записать резултат то просто возвращаем статус Ок в ОшибкаБаза.Код
*/
func (трз *Транзакция) Получить(ключПоиска ПервичныйКлюч, результат interface{}) (interface{}, ОшибкаБазы) {
	...
	// если не передан объект в который нужно вернуть найденое значение, то просто возвращаем код Ок и сообщаем что ключ существует, значение не важно, иначе если передан обхект, то получаем значение и вовзращаем
...
*/
func (трз *Транзакция) НайтиЗначение(ключ string, значение string) (map[string]Документ, ОшибкаБазы) {
	...
	найденныеДанные := make(map[string]Документ)
	...
	for итератор.Rewind(); итератор.Valid(); итератор.Next() {
		...
		err := объект.Value(func(значениТекущейИтерации []byte) error {
			Инфо("  %+s \n", значениТекущейИтерации)
			значениеВОбъекте := НайтиВJson(значениТекущейИтерации, "Данные."+ключ)
