...
func СоздатьJWT(ИдКлиента string, СекретКлиента Секрет) (string, СтатусСервиса) {
	...
	if СекретКлиента.Секрет == "" {
		var статус СтатусСервиса
		СекретКлиента, статус = ПолучитьСекретныйКлючКлиента(ИдКлиента)
		if статус.Код != Ок {
			return "", статус
		}
	}

	return ПодписатьJWT(требование, СекретКлиента.Секрет)
}
...
func ПолучитьСекретныйКлючКлиента(ИдКлиента string) (Секрет, СтатусСервиса) {
	...
	if статусСекрета.Код != Ок {
		if статусСекрета.Код == СрокСекретаИстекает {
			новвыйСекрет, статусСоздания := СоздатьСекретКлиента(ИдКлиента)
...
func ВлаидацияТокена(сообщение *Сообщение) (bool, СтатусСервиса) {
	...
	if сообщение.JWT == "" && сообщение.ТокенКлиента.ИдКлиента == uuid.Nil {
		return false, СтатусСервиса{
			...
			Текст: "Клиет не опознан, нет данных jwt и ИдКлиента",
		}
	}
	...
	// секрет, статус := ПолучитьСекретныйКлючКлиента(сообщение.ИдКлиента.String())
	// if статус.Код != Ок {
	// 	Ошибка("  %+v \n", статус)
	// }
	...
	токен, err := jwt.Parse(сообщение.JWT, func(token *jwt.Token) (interface{}, error) {
		...
		// if !ok {
		// 	return nil, errors.New("не удалось получить данные токена")
		// }
		// ИдКлиента, ok := claims["UID"].(string)
		картаJWT := token.Claims.(ПараметрыКлиента)
		Инфо("token.Claims картаJWT %+v \n", картаJWT)

		if картаJWT.ИдКлиента == "" {
			return nil, errors.New("нет ИдКлиента в JWT")
		}

		секрет, статус := ПолучитьСекретныйКлючКлиента(картаJWT.ИдКлиента)
		if статус.Код != Ок {
			Ошибка("  %+v \n", статус)
			return nil, errors.New(статус.Текст)
		}
