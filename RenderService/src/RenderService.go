...
func init() {
	...
	НаборШаблонов.ДобавитьФункции(jetРендерФункции())
...
func ОбработчикЗапросовСервера(поток quic.Stream, сообщение Сообщение) {
	Инфо("  ОбработчикЗапросовСервера входящее сообщение  %+v\n", сообщение)
	...
	*/
	...
	// ПарсингШаблонов()
	РендерОтветКлиенту(&сообщение) // записывает в ответКлиенту отрендеренные html
	// html := Рендер()
